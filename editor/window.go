...
func (w *Window) update() {
	...
	for i := start; i < end; i++ {
		...
		if drawWithSingleRect {
			...
			for j, _ := range contentMaskI {
				w.contentMaskOld[i][j] = contentMaskI[j]
			}
		} else {
...
func (w *Window) newBgCache(lastHighlight *Highlight, length int) *gui.QPixmap {
	font := w.getFont()
...
func (w *Window) setBgCache(highlight *Highlight, length int, image *gui.QPixmap) {
	if w.font != nil {
		w.cache.set(
			HlBgKey{
				bg:     highlight.bg(),
